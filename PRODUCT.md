...
- Photographs are never published straight from the camera. `process_images.py` resizes anything dropped into `whatsapp-items/` into a thumbnail and a display copy carrying no metadata, then deletes the original, since GPS coordinates in a photo taken at home would publish that address. The `ITEMS` list is the authority on which photographs exist: copies of anything it no longer references are deleted, and a photo it does not reference yet is refused rather than compressed
